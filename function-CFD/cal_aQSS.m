function cal = cal_aQSS(t, T, p, y_i, reaction,mix) 
Ru = 8.31442;
Rc = 1.98718;
...
Mw =mix.Mw; ns= mix.ns;D = mix.D;R_i = mix.R_i;
...
while(flag)
    ...
    sigma = max(abs((yc - yp)./((yc+e1)*c0*10^-2)));
    ...
    if sigma < 2
        ...
        dTdt = (dot(h_i,dydt)-T*dot(R_i,dydt))/(-dot(cp_i,y0)+dot(R_i,y0));
        T = T+ dTdt*dt;
